        ...
        <table id="paymentsTable" class="table table-striped rounded-top" style="width: auto; font-size: 12px;">
            ...
            <tbody>
                @foreach($payments as $payment)
                    <tr data-allocated="{{ $payment->receipts->first()->allocated_amount }}" data-remaining="{{ $payment->receipts->first()->remaining_amount }}" data-payment="{{ $payment->payment_amount }}">
                        <td>{{ $payment->id }}</td>
    ...
    function filterTable() {
        ...
        rows.forEach(function(row) {
            ...
            if (filterValue === 'allocated') {
                ...
                if (allocatedAmount > 0 && remainingAmount > 0) {
                    row.style.display = '';
                } else {
                    row.style.display = 'none';
                }
